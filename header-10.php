...
(function(){
  const track = document.getElementById("ogxLatestTrack");
  if(!track) return;

  const wrap = track.closest(".ogx-latestRail__wrap");
  const btnR = wrap.querySelector(".ogx-latestRail__arrow--right");
  const btnL = wrap.querySelector(".ogx-latestRail__arrow--left");
  ...
  const updateArrowState = () => {
    ...
    // 左右端なら矢印を薄く（表示ロジック自体はhoverで出る）
    if(btnL) btnL.style.opacity = (x <= 2) ? "0" : "";
    if(btnR) btnR.style.opacity = (x >= max - 2) ? "0" : "";
  };
  ...
  document.addEventListener("click", (e) => {
    const tgl = e.target.closest(".ogx-latestCard__toggle");
    if(!tgl) return;
